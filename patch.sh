...
perl -0pi -e 'if (!/public boolean openOptionsPage\(String extensionId\)/) { s|    /\*\* Returns the icon for the given extension index from native\. \*/|    /** Opens the extension options page if the extension exposes one. */\n    public boolean openOptionsPage(String extensionId) {\n        return ExtensionsMenuBridgeJni.get()\n                .openOptionsPage(mNativeExtensionsMenuDelegateAndroid, extensionId);\n    }\n\n    /** Returns the icon for the given extension index from native. */| }' chrome/browser/ui/android/extensions/java/src/org/chromium/chrome/browser/ui/extensions/ExtensionsMenuBridge.java
perl -0pi -e 'if (!/boolean openOptionsPage\(\n                long nativeExtensionsMenuDelegateAndroid,/) { s|        /\*\* Returns the icon for an extension'\''s action at actionIndex\. \*/|        /** Opens the extension options page if the extension exposes one. */\n        boolean openOptionsPage(\n                long nativeExtensionsMenuDelegateAndroid,\n                \@JniType("std::string") String extensionId);\n\n        /** Returns the icon for an extension'\''s action at actionIndex. */| }' chrome/browser/ui/android/extensions/java/src/org/chromium/chrome/browser/ui/extensions/ExtensionsMenuBridge.java
sed -i 's|                ("std::string") String extensionId);|                @JniType("std::string") String extensionId);|' chrome/browser/ui/android/extensions/java/src/org/chromium/chrome/browser/ui/extensions/ExtensionsMenuBridge.java
