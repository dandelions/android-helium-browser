...
# final normalize: keep exactly one getCurrentWebContents helper, including one-line legacy variants
python3 - "$MENU_MEDIATOR" <<'PYCODE'
from pathlib import Path
import sys
...
# ultimate normalize: remove every legacy getCurrentWebContents block by method boundary
python3 - "$MENU_MEDIATOR" <<'PYCODE'
from pathlib import Path
import re
import sys
