...
grep -q 'DEFINE_JNI_ExtensionsMenuBridge' "$ACTION_DELEGATE_CC" || perl -0pi -e 's~(#include "chrome/browser/ui/android/extensions/jni_headers/ExtensionsMenuBridge_jni.h"\n)~$1DEFINE_JNI_ExtensionsMenuBridge();\n~' "$ACTION_DELEGATE_CC"
perl -0pi -e 's|DEFINE_JNI_ExtensionsMenuBridge\(\);?\n|DEFINE_JNI_ExtensionsMenuBridge();\n|g; s|(DEFINE_JNI_ExtensionsMenuBridge\(\);\n){2,}|DEFINE_JNI_ExtensionsMenuBridge();\n|g' "$ACTION_DELEGATE_CC"
