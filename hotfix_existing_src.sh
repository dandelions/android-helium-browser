...
grep -q 'private long mLastMenuDismissedAtMs;' "$MENU_COORDINATOR" || \
    sed -i '/ExtensionsMenuMediator mMediator;/a\    private long mLastMenuDismissedAtMs;' "$MENU_COORDINATOR"
perl -0pi -e 's|(mExtensionsMenuButton\.setOnClickListener\(\n                \(view\) -> \{\n)(?!                    if \(android\.os\.SystemClock\.uptimeMillis\(\) - mLastMenuDismissedAtMs)|$1                    if (android.os.SystemClock.uptimeMillis() - mLastMenuDismissedAtMs\n                            < android.view.ViewConfiguration.getDoubleTapTimeout()) {\n                        return;\n                    }\n|' "$MENU_COORDINATOR"
perl -0pi -e 's|(public void onPopupMenuDismissed\(\) \{\n)(?!                        mLastMenuDismissedAtMs)|$1                        mLastMenuDismissedAtMs = android.os.SystemClock.uptimeMillis();\n|' "$MENU_COORDINATOR"
perl -0pi -e 's|(if \(mMediator != null\) \{\n                        mExtensionsMenuButton\.dismiss\(\);\n                        destroyMediator\(\);)\n                        mExtensionModels\.clear\(\);|$1|; s|(if \(android\.os\.SystemClock\.uptimeMillis\(\) - mLastMenuDismissedAtMs\n                            < android\.view\.ViewConfiguration\.getDoubleTapTimeout\(\)\) \{\n                        return;\n                    \}\n)(?!                    if \(mMediator != null\))|$1                    if (mMediator != null) {\n                        mExtensionsMenuButton.dismiss();\n                        destroyMediator();\n                        return;\n                    }\n|' "$MENU_COORDINATOR"
